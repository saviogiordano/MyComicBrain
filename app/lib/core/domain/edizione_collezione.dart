...
/// #112/#113): la cover va richiesta a parte via
/// `ComicsRepository.watchHydratazioneCollezione` solo per la finestra
/// caricata (vedi [EdizioneCollezioneFinestra]), e non è mai un campo di
/// questo tipo — evita che un `coverImage` nullable qui significhi sia
/// "non ancora hydratata" sia "nessuna cover", due cose diverse.
...
/// (§9, deciso su #112/#115): un'[EdizioneCollezioneIndice] più lo stato
/// della sua cover, hydratata separatamente e in ritardo rispetto
/// all'indice (`ComicsRepository.watchHydratazioneCollezione`).
///
/// [coverHydratata] distingue "non ancora hydratata" (query in corso per
/// questa Edizione, appena entrata in finestra) da "hydratata, nessuna
/// cover impostata" ([coverImage] `null` con [coverHydratata] `true`) —
/// stessa ragione del tipo distinto [EdizioneCollezioneIndice] rispetto a
/// un `coverImage` nullable unico.
class EdizioneCollezioneFinestra {
  const EdizioneCollezioneFinestra({
    required this.indice,
    required this.coverImage,
    required this.coverHydratata,
  });

  final EdizioneCollezioneIndice indice;
  final String? coverImage;
  final bool coverHydratata;

  int get edizioneId => indice.edizioneId;
  String get titolo => indice.titolo;
  String? get serieName => indice.serieName;
  String? get publisher => indice.publisher;
