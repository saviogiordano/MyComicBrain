...
void main() {
  ...
  Future<void> pumpImpostazioni(WidgetTester tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [settingsRepositoryProvider.overrideWithValue(repo)],
        child: MaterialApp(theme: AppTheme.dark, home: const ImpostazioniPage()),
      ),
    );
    await tester.pumpAndSettle();
  }

  testWidgets('senza provider salvato mostra il default Claude', (tester) async {
  ...
  testWidgets('modificare la API key AI la persiste nel repository', (tester) async {
    await pumpImpostazioni(tester);
  ...
  testWidgets('cambiare provider persiste la scelta e ricarica la sua API key', (
    tester,
  ) async {
  ...
  testWidgets('URL non valido per il provider Locale mostra un errore e non salva', (
    tester,
  ) async {
    ...
    await tester.enterText(find.byType(TextField), 'http://localhost:11434/v1');
    await tester.tap(find.text('Salva'));
    await tester.pumpAndSettle();
  ...
  testWidgets('modificare la API key ComicVine la persiste nel repository', (tester) async {
    await pumpImpostazioni(tester);
